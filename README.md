...
- **Real Satellite Cloud Cover**: The "Clouds" toggle overlays a dedicated cloud sphere with **NASA GIBS VIIRS Clear Sky Confidence** (Suomi NPP primary; the same-date NOAA-20 raster fills SNPP's no-data swath gaps pixel-by-pixel) — the real VIIRS cloud mask (confidence of *clear* sky, 1.0 = confident clear), converted to cloud confidence and passed through a conservative presentation transfer (ambiguous detections suppressed, max opacity capped so the map stays readable). Day + faint night sides, refreshed per load. The stylized day/night Earth stays underneath; no-data is transparent (never black). Clouds and daylight respond to zoom: at global view clouds are fully visible over the real solar day/night; one meaningful zoom inward (3.0–3.8 earth radii) fades the clouds and lifts the night side to full daylight. The Clouds feed chip shows source · imagery date · DAY+NIGHT.
...
- **Natural Earth geography** (`Geographic Context` in the Layers tab, ON by default): country borders, first-level administrative boundaries (US states, Brazilian states, Canadian provinces, German Länder, Indian states, Japanese prefectures, … across 51 countries), country/state names with postal/ISO short codes, and a 7,342-place city hierarchy (national capitals → world cities → admin-1 capitals → large centers → small towns) — built offline from Natural Earth v5.1.2 (public domain) into a ~1.41 MB bundled dataset (≈391 KB gzipped, `npm run build:geo`).
- **Semantic zoom LOD**: global view shows only major country names + faint borders; zooming in reveals admin-1 short codes (CA, TX, ON, BY …) → full state/province names + borders + national and admin-1 capitals → progressively richer local city coverage, with country labels fading out as subdivisions take over — all gates ramp smoothly on the same camera-distance metric (earth radii) as the cloud/daylight blend, but with their own semantic band thresholds (3.55 / 2.8 / 2.15 / 1.65).
- **Clean rendering**: batched line geometry (one material per layer — no per-country objects), labels on a dedicated canvas with per-zoom-band priority + grid-indexed collision decluttering, exact unit-sphere horizon visibility (`P · camera > 1`) + projected tangent-circle silhouette clipping (no far-side labels, nothing bleeds past the Earth silhouette), and tactical markers (cable stations, launch pads, nuclear plants, DSN sites, the target reticle) reserve their screen space so cartographic text never covers them.
- Verified by `npm run verify:geo` (city-in-country checks for every shipped city, representative world cities, dateline safety, projection convention pins).
- **Dev harness**: `window.__earthDebug` (camera + geographic-context label stats) exists only in dev builds (`import.meta.env.DEV`) — production bundles contain zero references.
