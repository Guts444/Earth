...
- **Natural Earth geography** (`Geographic Context` in the Layers tab, ON by default): country borders, first-level administrative boundaries (US states, Brazilian states, Canadian provinces, German Länder, Indian states, Japanese prefectures, … across 51 countries), country/state names with postal/ISO short codes, and a 7,342-place city hierarchy (national capitals → world cities → admin-1 capitals → large centers → small towns) — built offline from Natural Earth v5.1.2 (public domain) into a ~1.4 MB bundled dataset (`npm run build:geo`).
- **Semantic zoom LOD**: global view shows only major country names + faint borders; zooming in reveals admin-1 short codes (CA, TX, ON, BY …) → full state/province names + borders + national and admin-1 capitals → progressively richer local city coverage, with country labels fading out as subdivisions take over — all ramping smoothly against the same 3.0–3.8-radii transition that fades the clouds and lifts the night side.
- **Clean rendering**: batched line geometry (one material per layer — no per-country objects), labels on a dedicated canvas with per-zoom-band priority + grid-indexed collision decluttering, exact horizon + projected-tangent-circle occlusion (no far-side labels, nothing bleeds past the Earth silhouette), and tactical markers (cable stations, launch pads, nuclear plants, DSN sites, the target reticle) reserve their screen space so cartographic text never covers them.
